...
pub(crate) fn parse_vin_nospace(response: &str) -> Vec<u8> {
    ...
    if let Some(start) = hex_str.find("4902") {
        ...
        let data_start = if data_start + 2 <= hex_str.len() {
            data_start + 2
        } else {
            data_start
        };
        return (data_start..hex_str.len())
            .step_by(2)
            .filter_map(|i| {
                if i + 2 <= hex_str.len() {
...
pub(crate) fn validate_vin(bytes: Vec<u8>) -> String {
    ...
    if vin.len() == 17 {
        if vin.chars().all(|c| c != 'I' && c != 'O' && c != 'Q') {
            ...
            let filtered: String = vin
                .chars()
                .filter(|c| *c != 'I' && *c != 'O' && *c != 'Q')
                .collect();
            if filtered.len() == 17 {
                filtered
            } else {
        ...
        dev_log::log_warn(
            "vin_parser",
            &format!("VIN parse: got {} chars instead of 17: {}", vin.len(), vin),
        );
        String::new()
    }
}
...
pub(crate) fn is_valid_vin(vin: &str) -> bool {
    ...
    let sum: u32 = chars
        .iter()
        .enumerate()
        .map(|(i, &c)| transliterate(c) * weights[i])
        .sum();
    let check = sum % 11;
...
mod tests {
    ...
    fn test_validate_vin_with_nonascii() {
        let bytes = vec![
            0x56, 0x46, 0x33, 0xFF, 0x4C, 0x43, 0x42, 0x48, 0x5A, 0x36, 0x4A, 0x53, 0x30, 0x30,
            0x30, 0x30, 0x30,
        ];
        let vin = validate_vin(bytes);
